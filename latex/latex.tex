...
为进一步验证Path-LLM式拓扑-语义融合在道路段级路径表示中的有效性,本文进一步构建了5K规模的OSM道路段数据子集.具体而言,将PKDD15 Porto出租车GPS折线近似匹配到OSM drive路网,得到道路段序列、道路属性文本和道路段级动态特征.在排除GPT-2时使用2层Transformer编码器替代骨干网络,所有序列模型使用128维隐藏层,训练10个epoch.表\ref{tab:neural_results}给出序列模型的对比结果.

\begin{table}[htbp]
	\centering
	\caption{序列模型在5K OSM道路段子集上的TTE结果（未使用GPT-2, hidden\_dim=128）}
	\label{tab:neural_results}
	\begin{tabular}{lcccc}
		\toprule
		模型 & MAE/s & RMSE/s & MAPE & MARE \\
		\midrule
		LSTM (static embedding) & \textbf{190.85} & \textbf{247.07} & \textbf{0.3790} & \textbf{0.3263} \\
		PathLLM-Static w/ OSM text & 281.41 & 394.00 & 0.4014 & 0.4812 \\
		Transformer (static embedding) & 316.85 & 427.67 & 0.4500 & 0.5418 \\
		PathLLM-Static w/o text & 351.72 & 458.37 & 0.5083 & 0.6014 \\
		\bottomrule
	\end{tabular}
\end{table}

从5K OSM道路段子集上的结果来看：(1) LSTM作为轻量级序列基线取得最佳TTE性能(MAE 190.85s),说明在小数据场景下,参数较少的循环网络仍具有很强竞争力.(2) PathLLM-Static w/ OSM text的MAE为281.41s,优于普通Transformer的316.85s,说明道路段文本语义和拓扑/语义融合能够补充纯序列编码器的表示能力.(3) 移除道路文本后,PathLLM-Static的MAE由281.41s退化到351.72s,表明真实OSM道路文本是当前道路段级Path-LLM式模型中的关键有效模态.这一结果比网格token上的伪语义实验更直接地验证了Path-LLM论文中道路文本语义对路径表示学习的价值.

需要说明的是,OSM 5K序列实验的绝对MAE数值与全量表格模型不可直接比较,因为二者的数据规模、token定义、输入模态和模型结构均不同.本文将该实验定位为小规模机制验证:它主要用于回答真实道路文本是否有效、Path-LLM式拓扑-语义融合是否优于普通Transformer、以及动态融合策略在小数据下是否稳定.
...
在OSM 5K道路段子集上,本文进一步对静态PathLLM式模型、简单拼接和普通门控进行小规模消融.表\ref{tab:fusion_summary}总结了融合策略之间的相对关系.结果表明,SimpleGate(294.48s)优于Concat(300.65s),说明门控式自适应加权比简单拼接更适合处理静态语义表示和动态状态表示.然而,SimpleGate仍弱于PathLLM-Static(281.41s),说明当前5K小样本条件下,动态特征的噪声和稀疏性尚未带来稳定增益.这与表格模型中的全量结果并不矛盾:动态状态对数据规模、历史观测密度和token稳定性更加敏感,需要在更充分的训练样本或更可靠的map matching结果上进一步验证.

\begin{table}[htbp]
	\centering
	\caption{融合策略在5K OSM道路段子集上的对比}
	\label{tab:fusion_summary}
	\begin{tabular}{lcc}
		\toprule
		对比维度 & 对比结果 & MAE差距/s \\
		\midrule
		文本模态是否带来增益 & w/o text(351.72) vs w/ text(281.41) & +70.31 \\
		PathLLM式融合 vs 普通Transformer & Transformer(316.85) vs PathLLM-Static(281.41) & +35.44 \\
		门控 vs 拼接 & Concat(300.65) vs SimpleGate(294.48) & +6.17 \\
		动态模态的小数据表现 & StaticOnly(281.41) vs SimpleGate(294.48) & -13.07 \\
		\bottomrule
	\end{tabular}
\end{table}

上述分析围绕三个研究问题展开实验验证:(1)道路文本语义是否有效——w/ text相较w/o text降低70.31s MAE,给出了最直接的正面证据;(2)PathLLM式拓扑-语义融合是否优于普通Transformer——PathLLM-Static相较Transformer降低35.44s MAE,说明道路语义对路径表示有实际贡献;(3)动态融合策略是否稳定——SimpleGate相较Concat降低6.17s MAE,但仍弱于StaticOnly,说明门控结构本身有一定价值,而动态模态在小规模道路段数据上还需要更充分的历史观测和更稳定的动态状态估计.
...
在真实出租车轨迹数据集上的旅行时间估计实验表明,动态增强模型相较静态模型能够稳定降低预测误差.其中,在113K全量数据上,HGB DynaPath-Lite取得85.22s MAE,较静态HGB的88.60s提升约3.8\%;在Ridge线性模型中,动态特征也带来约5.8\%的MAE改进(97.42s→91.79s).消融实验表明,速度相关特征贡献最大,动态质量信号(观测密度、时效性、可靠性)提供稳定但较小的辅助作用.在sparse-30\%动态缺失场景下,动态信息大幅退化(MAE由85.22s退至88.41s),验证了动态数据稀疏性问题对模型性能的显著影响.在5K OSM道路段子集上,神经序列模型实验表明:LSTM作为轻量级序列基线取得最佳性能;PathLLM-Static引入真实OSM道路文本后,MAE由351.72s降至281.41s,并优于普通Transformer的316.85s,验证了道路文本语义对Path-LLM式路径表示的价值.在动态融合方面,SimpleGate优于Concat(294.48s vs 300.65s),但仍劣于静态PathLLM模型,说明动态模态在小规模道路段数据上仍受历史观测稀疏和状态估计噪声影响.
...
本文仍存在若干局限.当前OSM道路段实验采用最近节点吸附和最短路桥接的近似map matching方法,尚未使用HMM式地图匹配器;序列模型实验也仅在5K子集上以128维小模型进行了初步验证.全量道路段级序列模型训练、真实GPT-2骨干集成以及更严格的动态缺失实验需要在配备GPU的服务器上完成.下一步将改进OSM map matching质量,引入node2vec拓扑嵌入和更强的道路文本编码器,在充足数据和更大规模模型上完成DynaPathLLM全量序列实验,并在旅行时间估计、路径排序和拥堵预测等多个下游任务上评估.所有模型代码、训练入口、分析工具和实验管理脚本均已在本文的代码仓库中提供,可供后续研究和评审复现.
