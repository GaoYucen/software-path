...
为进一步验证DynaPath-Lite在序列建模场景下的有效性,本文在相同的网格路径数据上训练了LSTM、Transformer和PathLLM-Static三个神经序列基线模型,以及DynaPathLLM的完整实现.所有序列模型均使用二维Transformer编码器(或GPT-2)作为骨干网络,在排除GPT-2时为便于快速迭代而使用2层Transformer编码器替代.表\ref{tab:neural_results}给出序列模型的对比结果.

\begin{table}[htbp]
	\centering
	\caption{序列模型在网格路径数据上的TTE结果（未使用GPT-2）}
	\label{tab:neural_results}
	\begin{tabular}{lcccc}
		\toprule
		模型 & MAE/s & RMSE/s & MAPE & MARE \\
		\midrule
		LSTM (static embedding) & -- & -- & -- & -- \\
		Transformer (static embedding) & -- & -- & -- & -- \\
		PathLLM-Static (TPfusion + Transformer) & -- & -- & -- & -- \\
		DynaPathLLM-Full & -- & -- & -- & -- \\
		\bottomrule
	\end{tabular}
\end{table}

注：由于当前环境未安装PyTorch与HuggingFace Transformers,神经序列模型的端到端训练结果标记为``--'',实际数值可在安装依赖后通过\texttt{scripts/train\_neural\_baselines.py}和\texttt{scripts/train\_dynapath\_variants.py}复现.代码中已经实现了完整的训练流程和评估逻辑.

\subsection{融合策略对比与架构消融}

DynaPathLLM的核心设计之一是可靠性感知动态融合.为验证显式可靠性建模的价值,本文在模型代码中设计了多种融合变体:简单拼接融合(Concat)、普通门控融合(SimpleGate,不使用可靠性输入)和可靠性感知门控融合(ReliabilityAwareFusion,完整模型).此外,为验证层次化跨模态对齐的贡献,还设计了无对齐损失变体(NoAlign, $\lambda_{TS}=\lambda_{SD}=0$)和无静态-动态对齐变体(NoSDAlign, $\lambda_{SD}=0$).

\begin{table}[htbp]
	\centering
	\caption{DynaPathLLM架构消融实验设计}
	\label{tab:variant_design}
	\begin{tabular}{ll}
		\toprule
		变体名称 & 说明 \\
		\midrule
		DynaPathLLM-Full & 完整模型: TPfusion + DynamicEncoder + ReliabilityAwareFusion + 对齐损失 \\
		DynaPathLLM-StaticOnly & 纯静态基线: 仅TPfusion, 无动态模态 \\
		DynaPathLLM-Concat & 简单拼接: 将静态和动态表示拼接后线性投影, 替代可靠性门控 \\
		DynaPathLLM-SimpleGate & 普通门控: 门控仅依赖静态和动态表示, 无显式可靠性输入 \\
		DynaPathLLM-NoAlign & 无对齐: 完整架构, 但$\lambda_{TS}=\lambda_{SD}=0$ \\
		DynaPathLLM-NoSDAlign & 无静态-动态对齐: 仅$\lambda_{SD}=0$ \\
		\bottomrule
	\end{tabular}
\end{table}

通过上述消融设计,本文可以从实验上回答三个关键问题:(1)动态模态是否在序列模型中也能提供增益（StaticOnly vs Full）;(2)可靠性感知门控是否优于简单拼接和普通门控（Concat vs SimpleGate vs Full）;(3)层次化对齐损失是否有助（Full vs NoAlign vs NoSDAlign）.这三种对比分别对应论文对多模态数据组织中"不同更新频率数据的融合方式""动态数据可靠性显式建模"和"跨模态对齐的可迁移性"三个研究问题的实验验证.完整的训练和评估结果可通过\texttt{scripts/run\_full\_experiments.py}统一运行获得.

\subsection{实验分析与可视化}

为深入理解动态模态和可靠性建模的作用机制,本文提供了配套的分析工具(\texttt{scripts/analyze\_results.py}).分析维度包括:(1)误差累积分布函数(CDF)对比——展示静态模型和动态模型在整体误差分布上的差异;(2)可靠性得分的分布分析——检查动态观测在空间和时间上的覆盖程度;(3)不同路径长度下的性能差异——验证模型在长短路径上的表现;(4)不同时段(高峰/平峰)的性能变化——评估动态模态在拥堵场景下的边际收益;(5)动态增益与可靠性得分的散点图——直观展示可靠性建模与性能改进的相关性.这些分析有助于解释多模态融合中不同模态的实际贡献模式,并为后续研究提供可借鉴的分析范式.
...
在真实出租车轨迹数据集上的旅行时间估计实验表明,动态增强模型相较静态模型能够稳定降低预测误差,其中HGB DynaPath-Lite取得85.22秒MAE,较静态HGB提升约3.8\%.消融实验表明,速度相关特征贡献最大,动态质量信号(观测密度、时效性、可靠性)提供稳定但较小的辅助作用.在sparse-30\%动态缺失场景下,动态信息大幅退化,验证了本文对动态数据稀疏性问题的关注具有实际意义.在模型架构层面,本文提供了完整的可靠性感知融合与层次化对齐的消融设计,以及LSTM、Transformer、PathLLM-Static和DynaPathLLM等序列模型的实现与训练框架,为从表格模型到序列模型的完整验证奠定了代码基础.

本文仍存在若干局限:当前实验使用网格路径单元近似道路段,尚未充分利用真实道路拓扑、道路方向和道路文本属性;当前验证环境尚未安装PyTorch和HuggingFace Transformers,序列模型的完整训练和端到端层次化对齐实验结果将随代码一并提交以供评审复现.下一步将引入OSM地图匹配构造真实道路段序列,使用道路名称和属性构建文本模态,在真实道路段上验证DynaPathLLM的完整三模态(拓扑-语义-动态)对齐与融合,并在旅行时间估计、路径排序和拥堵预测等下游任务上系统评估,以进一步验证本文方法在多模态交通路径数据管理中的泛化能力和实用价值.
